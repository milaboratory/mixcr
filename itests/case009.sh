...
assert "cat case9.align.report.json | head -n 1 | jq -r .chainUsage.chains.TRA.total" "241354"
assert "cat case9.assemble.report.json | head -n 1 | jq -r .readsInClones" "200759"
assert "cat case9.assembleContigs.report.json | head -n 1 | jq -r .longestContigLength" "499"
assert "cat case9.assembleContigs.report.json | head -n 1 | jq -r .clonesWithAmbiguousLetters" "3559"
assert "cat case9.assembleContigs.report.json | head -n 1 | jq -r .assemblePrematureTerminationEvents" "4"
assert "cat case9.assembleContigs.report.json | head -n 1 | jq -r .finalCloneCount" "25655"
