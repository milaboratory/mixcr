...
mixcr analyze --verbose 10x-sc-xcr-vdj-fast \
  --species hs \
...
assert "mixcr exportClones --no-header base_single_cell.vdjcontigs.assembledCells.clns | grep -v '^WARNING' | wc -l" "7"
assert "mixcr exportClones --no-header --split-by-tags Cell base_single_cell.vdjcontigs.assembledCells.clns | grep -v '^WARNING' | wc -l" "7"
assert "mixcr exportClones --no-header --split-by-tags Molecule base_single_cell.vdjcontigs.assembledCells.clns | grep -v '^WARNING' | wc -l" "228"
assert "mixcr exportClones --no-header -tags Molecule base_single_cell.vdjcontigs.assembledCells.clns | grep -v '^WARNING' | wc -l" "228"
assert "mixcr exportClones --no-header --drop-default-fields -cellGroup base_single_cell.vdjcontigs.assembledCells.clns | grep -v '^WARNING' | sort | uniq | wc -l" "3"
