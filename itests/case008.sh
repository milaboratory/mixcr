...
assert "cat case8.align.report.json | head -n 1 | jq -r .chainUsage.chains.TRA.total" "241354"
assert "cat case8.assemble.report.json | head -n 1 | jq -r .readsInClones" "200759"
assert "cat case8.assemble.report.json | head -n 1 | jq -r .clones" "25655"
assert "cat case8.assembleContigs.report.json | head -n 1 | jq -r .longestContigLength" "499"
assert "cat case8.assembleContigs.report.json | head -n 1 | jq -r .clonesWithAmbiguousLetters" "3559"
assert "cat case8.assembleContigs.report.json | head -n 1 | jq -r .assemblePrematureTerminationEvents" "4"
assert "cat case8.assembleContigs.report.json | head -n 1 | jq -r .finalCloneCount" "25655"
