...
object CommandAnalyze {
    ...
    class Cmd : AbstractMiXCRCommand() {
        // @Parameters(description = ["preset_name input_file1 [input_file2] analysisOutputName"], arity = "3..4")
        ...
        @Parameters(
            index = "1",
            arity = "2..3",
            // paramLabel = "inputs and outputs",
            description = ["Path to output table. Print in stdout if omitted."]
        )
        var inOut: List<String> = mutableListOf()

        val inFiles get() = inOut.dropLast(1)
        val outputSuffix get() = inOut[inOut.size - 1]
        ...
        //     description = ["Path to output table. Print in stdout if omitted."]
        // )
        // lateinit var inFiles: List<String>
        //
        // @Parameters(
        //     index = "2",
        //     arity = "1",
        //     paramLabel = "output_suffix",
        //     description = ["Path to output table. Print in stdout if omitted."]
        // )
        // lateinit var outputSuffix: String
        ...
        val paramsResolver = object : MiXCRParamsResolver<MiXCRPipeline>(this, MiXCRParamsBundle::pipeline) {
        ...
        override fun run0() {
            ...
            var outputIsFolder = outputSuffix.endsWith(File.separator)
            val outputPath = Path(outputSuffix)
            ...
            planBuilder.addStep(MiXCRCommand.align, mixins.flatMap { it.cmdArgs })
            ...
            planBuilder.executionPlan.forEach { pe ->
                println(pe)
            }
        }
        ...
        data class ExecutionStep(
            ...
            override fun toString() =
                (listOf(command.command) + arguments + extraArgs + inputs + output)
                    .joinToString(" ")
        }
    }
}
