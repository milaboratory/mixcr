...
- kAligner2 has been modified to work with both TCR and BCR data, the presets previously divided by the type of data now merged into one. Also, the division by `cdr3` and `full-length` presets has been deprecated; now the presets by default use the longest possible assembling feature according to the protocol:
  ...
  - `smartseq2-vdj-full-length` is depricated and replaced by `smartseq2-vdj`.

  - `singleron-2.0.1-vdj-cdr3` is depricated and replaced by `singleron-2.0.1-vdj`.

  - `seqwell-vdj-cdr3` is depricated and replaced by `seqwell-vdj`.

  - `parse-bio-vdj-3gex` is depricated and replaced by `parse-bio-3gex`.

  - `split-seq-vdj-3gex` is depricated and replaced by `split-seq-3gex`.

  - `ont-rna-seq-vdj-full-length` is depricated and replaced by `ont-rna-seq-vdj`.
